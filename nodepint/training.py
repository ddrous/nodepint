...
def train_project_neural_ode(neural_nets:tuple, 
                             data:Dataset, 
                             pint_scheme:str, 
                             samp_scheme:str, 
                             coarse_integrator:callable, 
                             coarse_integrator_args:tuple, 
                             fine_integrator:callable, 
                             fine_integrator_args:tuple, 
                             loss_fn:callable, 
                             optim_scheme:GradientTransformation, 
                             nb_processors:int, 
                             nb_epochs:int, 
                             batch_size:int, 
                             scheduler:float, 
                             times: tuple, 
                             fixed_point_args:tuple, 
                             repeat_projection:int, 
                             nb_vectors:int, 
    ...
    print("Coarse integrator is: ", coarse_integrator.__name__)
    print("Fine integrator is: ", fine_integrator.__name__)
    ...
    for p in range(repeat_projection):
        ...
        if not neural_encoding:
            ...
            neural_nets = basis_wrap, neural_nets[1], neural_nets[2], neural_nets[3]
        ...
        neural_nets, loss_ht, errors, nb_iters = train_neural_ode(neural_nets, data, pint_scheme, shooting_function, nb_processors, times, coarse_integrator, coarse_integrator_args, fine_integrator, fine_integrator_args, fixed_point_args, loss_fn, optim_scheme, scheduler, nb_epochs, batch_size, force_serial)
...
def train_neural_ode(neural_nets, dataset, pint_scheme, shooting_fn, nb_processors, times, coarse_integrator, coarse_integrator_args, fine_integrator, fine_integrator_args, fixed_point_args, loss_fn, optim_scheme, scheduler, nb_epochs, batch_size, force_serial):
    ...
    for epoch in range(nb_epochs):
        ...
        for batch in dataloader:
            ...
            params, optstate, loss_val, aux_data = train_step(params, static, x, y, loss_fn, pint_scheme, shooting_fn, nb_processors, times, coarse_integrator, coarse_integrator_args, fine_integrator, fine_integrator_args, fixed_point_args, optimiser, optstate, force_serial)
...
@partial(jax.jit, static_argnames=("static", "loss_fn", "pint_scheme", "shooting_fn", "nb_processors", "times", "coarse_integrator", "coarse_integrator_args", "fine_integrator", "fine_integrator_args", "optimiser", "fixed_point_args", "force_serial"))
def train_step(params, static, x, y, loss_fn, pint_scheme, shooting_fn, nb_processors, times, coarse_integrator, coarse_integrator_args, fine_integrator, fine_integrator_args, fixed_point_args, optimiser, optstate, force_serial):

    (loss_val, aux_data), grad = jax.value_and_grad(node_loss_fn, has_aux=True)(params, static, x, y, loss_fn, pint_scheme, shooting_fn, nb_processors, times, coarse_integrator, coarse_integrator_args, fine_integrator, fine_integrator_args, fixed_point_args, force_serial)
...
def node_loss_fn(params, static, x, y, loss_fn, pint_scheme, shooting_fn, nb_processors, times, coarse_integrator, coarse_integrator_args, fine_integrator, fine_integrator_args, fixed_point_args, force_serial):
    ...
    if force_serial == False:
        ...
        rtol, atol, hmax, max_steps, max_steps_rev, kind = coarse_integrator_args
        ...
        rtol, atol, hmax, max_steps, max_steps_rev, kind = fine_integrator_args

        if fine_integrator.__name__ in ("euler_integrator", "rk4_integrator"):    ## Fixed steps sizes, users must know this !
    ...
    #     print("Shapes:", y.shape, y_pred.shape)
...
def test_neural_ode(neural_nets, data, pint_scheme, shooting_fn, nb_processors, times, integrator, integrator_args, fixed_point_args, acc_fn, batch_size):
    ...
    for batch in dataloader:
        ...
        acc_val, y_preds = test_step(params, static, x, y, acc_fn, pint_scheme, shooting_fn, nb_processors, times, integrator, integrator_args, fixed_point_args)

        nb_examples += x.shape[0]
        total_acc += acc_val

    return total_acc/nb_examples, (x, y, y_preds)
